#!/usr/bin/env python
"""
v6.4A 集中+行业分散 — 基于v6.3 (年化35.83%, 回撤-25.78%)

v6.3发现: 熔断从未触发(0次), 收益差距来自分散化参数(35只/8%上限)。
v6.4A策略: 恢复v6.0的集中持仓(30只), 去掉熔断器, 仅保留行业上限控制风险。

变化 vs v6.3:
  1. N_STOCKS: 35 → 30 (恢复v6.0集中度)
  2. MAX_WEIGHT: 0.08 → 0.15 (基本不限制, 让ML自由配)
  3. SECTOR_CAP: 0.25 → 0.30 (宽松行业上限)
  4. DD熔断: 完全去掉 (v6.3已证明无用)

逻辑: 行业分散是降低回撤最有效的手段, 个股集中度保留alpha。
目标: 年化>40% + 回撤<-28%
"""
...
MAX_WEIGHT = 0.15       # 单股上限15%(基本不限制)
MIN_WEIGHT = 0.008      # 单股下限0.8%
SECTOR_CAP = 0.30       # 行业上限30%(宽松行业分散)
...
# v6.4A: 完全去掉回撤熔断 (v6.3已证明无用)
VOL_LOOKBACK = 20       # 波动率回看天数(仅观察)
DD_THRESHOLD_1 = -0.99  # 禁用: 设为-99%永远不触发
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.4A 集中+行业分散 (动态动量池 + ML + 择时 + 宽松回撤熔断)")
    ...
    # 保存结果
    result = {
        'version': 'v6.4a_concentrated_sector_cap',
        'config': {
            'n_stocks': N_STOCKS,
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_4a.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.4A 集中+行业分散回测')
