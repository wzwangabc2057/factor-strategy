#!/usr/bin/env python
"""
v6.3 宽松熔断 — 基于v6.2 (年化32%, 回撤-23%)

v6.2教训: 熔断阈值-15%/-20%太敏感, 震荡市反复触发导致错失反弹, 年化损失13%。
v6.3策略: 放宽熔断阈值到-20%/-25%, 只在真正大跌时触发。

变化 vs v6.2:
  1. 熔断阈值放宽: -15%/-20% → -20%/-25% (减少误触发)
  2. 恢复阈值放宽: -10% → -12% (更快恢复)
  3. 其余与v6.2一致(35只, 8%上限)
...
DD_THRESHOLD_1 = -0.20  # 回撤熔断阈值1: -20% → 暴露减半
DD_THRESHOLD_2 = -0.25  # 回撤熔断阈值2: -25% → 暴露30%
DD_RECOVERY = -0.12     # 回撤恢复阈值: 回到-12%以内恢复正常
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.3 宽松熔断 (动态动量池 + ML + 择时 + 宽松回撤熔断)")
    ...
    # 保存结果
    result = {
        'version': 'v6.3_relaxed_circuit_breaker',
        'config': {
            'n_stocks': N_STOCKS,
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_3.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.3 宽松熔断回测')
