#!/usr/bin/env python
"""
v6.5B 个股止损 — 基于v6.4B (年化46.28%, 回撤-30.09%)

v6.3发现: 分散化参数固定损失~9%alpha, 但市场环境不同需要不同策略。
v6.4B策略: 根据宏观择时信号动态调整集中度。

逻辑:
  - 牛市(exposure>=0.90): 集中25只, 无权重上限 → 最大化alpha
  - 正常(exposure 0.75-0.90): 30只, 10%上限 → 平衡
  - 弱势(exposure<0.75): 40只, 5%上限 → 最大化分散防守

变化 vs v6.3:
  1. N_STOCKS: 固定35 → 动态25/30/40
  2. MAX_WEIGHT: 固定8% → 动态15%/10%/5%
  3. DD熔断: 完全去掉 (用动态分散替代)
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.5B 个股止损 (动态动量池 + ML + 择时 + 宽松回撤熔断)")
    ...
    peak_nav = 1.0
    current_nav = 1.0
    # v6.5B: 个股止损 — 跟踪每只股票的买入价
    stock_entry_prices = {}  # {code: entry_price}
    STOCK_STOP_LOSS = -0.15  # 个股从买入价跌15%止损
    stock_stop_count = 0
    ...
    for i in range(1, len(dates)):
        ...
        if current_weights and curr_date in price_pivot.index:
            ...
            for code, w in list(current_weights.items()):
                if w < 0.001:
                    continue
                if code in stock_entry_prices and code in curr_p.index and pd.notna(curr_p[code]):
                    entry_px = stock_entry_prices[code]
                    if entry_px > 0:
                        ret_from_entry = curr_p[code] / entry_px - 1
                        if ret_from_entry < STOCK_STOP_LOSS:
                            stopped_codes.append(code)
            if stopped_codes:
                # 止损: 移除这些股票, 权重重新分配给剩余持仓
    ...
    # v6.5B 个股止损统计
    print(f"{'个股止损次数':.<20} {stock_stop_count:>15d}")
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_5b.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.5B 个股止损回测')
