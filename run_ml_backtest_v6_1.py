#!/usr/bin/env python
"""
v6.1 回撤优化版 — 基于v6.0 (年化45%, 回撤-30%)

优化方向:
  1. 择时更激进: min_exposure 0.50→0.30, 允许更大幅度减仓
  2. 目标波动率: 15%年化目标, 超标自动缩仓
  3. 降集中度: 单股上限5%(原10%), N_STOCKS 30→40
  4. 回撤熔断: 回撤>15%减半仓, >20%降至30%
  5. 动量池质量: RSI过热惩罚 + 近期大跌剔除
...
# 组合参数
N_STOCKS = 40           # 持股数量 40只(原30, 更分散降回撤)
MAX_WEIGHT = 0.05       # 单股上限5%(原10%, 降集中度)
MIN_WEIGHT = 0.008      # 单股下限0.8%
SECTOR_CAP = 0.20       # 行业上限20%(原25%)
...
class MomentumPoolBuilder:
    ...
    def build_pool(self, as_of_date):
        ...
        # v6.1: 质量过滤
        # 1. RSI过热惩罚: RSI>80的股票降权20%
        rsi_penalty = np.where(score_df['rsi_14'] > 80, 0.80,
                      np.where(score_df['rsi_14'] > 70, 0.90, 1.00))
        score_df['composite_score'] *= rsi_penalty

        # 2. 近期大跌剔除: 20日内跌幅>25%的直接排除
        score_df = score_df[score_df['recent_dd_20d'] > -0.25]

        # 3. 极端波动惩罚: 年化波动>100%的降权
        vol_penalty = np.where(score_df['vol_60d'] > 1.0, 0.85,
                     np.where(score_df['vol_60d'] > 0.80, 0.92, 1.00))
        score_df['composite_score'] *= vol_penalty
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.1 回撤优化版 (动态动量池 + ML + 择时 + 波动率控制 + 回撤熔断)")
    print(f"回测期: {start_date} ~ {end_date}")
    print(f"配置: 动量池230只 → ML Top-{N_STOCKS} | 月度调仓 | 150+因子 | 波动率倒数加权")
    print(f"风控: 单股≤{MAX_WEIGHT:.0%} | 换手≤{MAX_TURNOVER:.0%}/月 | 目标波动率{TARGET_VOL:.0%}")
    ...
    timing_signal = MacroTimingSignal(min_exposure=0.30, max_exposure=1.00, ma_period=250)
    market_position = 1.0
    stop_loss_count = 0
    enhanced_returns = []
    original_returns = []
    ...
    for i in range(1, len(dates)):
        ...
        if len(enhanced_returns) >= VOL_LOOKBACK:
            realized_vol = np.std(enhanced_returns[-VOL_LOOKBACK:]) * np.sqrt(252)
            if realized_vol > TARGET_VOL and realized_vol > 0:
                vol_scalar = min(1.0, TARGET_VOL / realized_vol)

        # 综合暴露度 = 择时 × 回撤熔断 × 波动率缩放
        final_exposure = market_position * dd_circuit_breaker * vol_scalar
    ...
    # 保存结果
    result = {
        'version': 'v6.1_drawdown_optimized',
        'config': {
            'n_stocks': N_STOCKS,
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_1.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.1 回撤优化版回测')
