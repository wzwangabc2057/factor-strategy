...
基于v6.5A (年化47.74%, 回撤-26.47%), 叠加三重风控:
  1. 激进择时 (v6.5A): min_exposure=0.20, exposure^1.5放大
  2. 个股止损: 从买入价跌12%自动卖出
  3. 渐进式组合止损: NAV回撤线性减仓
     -8% → 70%仓位, -12% → 40%仓位, -15% → 20%仓位, -18% → 10%仓位
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.6A 三重保护 (激进择时 + 个股止损 + 渐进组合止损)")
    ...
    # 保存结果
    result = {
        'version': 'v6.6a_triple_protection',
        'config': {
            'n_stocks': N_STOCKS,
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.6A 三重保护回测')
