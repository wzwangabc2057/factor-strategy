#!/usr/bin/env python
"""
v6.5C 周度调仓 — 基于v6.4B (年化46.28%, 回撤-30.09%)

核心改动: 调仓频率从月度→周度(每5个交易日)
  - ML训练仍保持月度(数据量足够时), 但组合调仓每周执行
  - 更快响应市场变化, 减少持仓在崩盘中被动承受损失
  - 换手率会增加, 但交易成本已包含在回测中
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.5C 周度调仓 (动态动量池 + ML + 择时 + 宽松回撤熔断)")
    ...
    for y in range(train_start_dt.year, end_dt.year + 1):
        for m in range(1, 13):
            ...
            if train_start_dt.strftime('%Y-%m-%d') <= rd <= end_date:
                all_train_dates.append(rd)
    train_rebalance_dates = [d for d in all_train_dates if d < start_date]

    # 交易期周度调仓日期将在dates构建后生成 (见下方[4/6]部分)
    trade_rebalance_dates = []  # placeholder, 在dates构建后填充
    # 月初日期用于ML训练触发
    monthly_train_dates = set()
    for y in range(start_dt.year, end_dt.year + 1):
        for m in range(1, 13):
            rd = f'{y}-{m:02d}-01'
            if start_date <= rd <= end_date:
                monthly_train_dates.add(rd)
    ...
    # v6.5C: 生成周度调仓日期 (每5个交易日)
    trade_rebalance_dates = [d for idx, d in enumerate(dates) if idx % 5 == 0]
    logger.info(f"  周度调仓: {len(trade_rebalance_dates)}个调仓日 (每5个交易日)")
    ...
    for i in range(1, len(dates)):
        ...
        while rb_idx < len(trade_rebalance_dates) and curr_date >= trade_rebalance_dates[rb_idx]:
            ...
            try:
                ...
                # Step 4: 训练ML (仅月度触发, 需要≥MIN_TRAIN_MONTHS个月历史)
                # v6.5C: 找到最近的月初日期来判断是否需要训练
    ...
    # 保存结果
    result = {
        'version': 'v6.5c_weekly_rebalance',
        'config': {
            'n_stocks': N_STOCKS,
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_5c.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.5C 周度调仓回测')
