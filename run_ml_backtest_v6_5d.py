#!/usr/bin/env python
"""
v6.5D 波动率择时 — 基于v6.4B (年化46.28%, 回撤-30.09%)

核心改动: 添加波动率择时叠加层
  - 计算市场近20日实现波动率(年化)
  - 波动率 > 30%时, 将暴露度乘以0.5 (大幅减仓)
  - 波动率 > 25%时, 将暴露度乘以0.7 (适度减仓)
  - 波动率 <= 25%时, 不做额外调整
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.5D 波动率择时 (动态动量池 + ML + 择时 + 宽松回撤熔断)")
    ...
    for i in range(1, len(dates)):
        ...
        # v6.5D: 波动率择时叠加
        # 计算市场近20日实现波动率 (用沪深300指数收益)
        vol_multiplier = 1.0
        if i >= 20 and '000300.SH' in index_prices:
            idx_series = index_prices['000300.SH']
            recent_dates = [d for d in dates[max(0,i-20):i+1] if d in idx_series.index]
            if len(recent_dates) >= 10:
                idx_rets = idx_series.loc[recent_dates].pct_change().dropna()
                realized_vol = idx_rets.std() * np.sqrt(250)  # 年化波动率
                if realized_vol > 0.30:
                    vol_multiplier = 0.50  # 高波动: 大幅减仓
                elif realized_vol > 0.25:
                    vol_multiplier = 0.70  # 中等波动: 适度减仓
    ...
    print(f"\n{'v6.5D 波动率择时统计':}")
    print(f"  波动率叠加层: >30% → 0.5x, >25% → 0.7x")
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_5d.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.5D 波动率择时回测')
