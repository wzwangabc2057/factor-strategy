#!/usr/bin/env python
"""
v6.4C 增强波动率加权 — 基于v6.3 (年化35.83%, 回撤-25.78%)

v6.3发现: 已有波动率倒数加权, 但力度不够, 高波股仍占较大权重。
v6.4C策略: 恢复30只集中持仓, 增强波动率惩罚(vol^1.5), 不靠分散降回撤。

变化 vs v6.3:
  1. N_STOCKS: 35 → 30 (恢复v6.0集中度)
  2. MAX_WEIGHT: 0.08 → 0.12 (放宽限制)
  3. 波动率加权: 1/vol → 1/vol^1.5 (更强惩罚高波)
  4. DD熔断: 完全去掉

逻辑: 不牺牲持股数量, 但通过更强的波动率惩罚降低组合波动率。
目标: 年化>40% + 波动率更低 → 更好的夏普
"""
...
# 组合参数
N_STOCKS = 30           # 持股数量 30只(恢复v6.0集中度)
MAX_WEIGHT = 0.12       # 单股上限12%(放宽)
MIN_WEIGHT = 0.008      # 单股下限0.8%
SECTOR_CAP = 0.25       # 行业上限25%
VOL_PENALTY_POWER = 1.5 # 波动率惩罚指数 (原1.0, 增强到1.5)
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.4C 增强波动率加权 (动态动量池 + ML + 择时 + 宽松回撤熔断)")
    ...
    # 保存结果
    result = {
        'version': 'v6.4c_enhanced_vol_weighting',
        'config': {
            'n_stocks': N_STOCKS,
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_4c.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.4C 增强波动率加权回测')
