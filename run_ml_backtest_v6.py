...
核心进化: 动态动量池构建 (全市场~5000只 → 筛选230只) 替代静态Excel选股

策略链:
  1. 动量池: 全市场扫描 → 动量/波动/换手/均线多因子评分 → Top-230股票池
  2. ML排序: 150+因子 → LGB+XGB ensemble → Top-30集中持仓
  3. 择时: v5.3多信号宏观择时 (CSI300趋势+波动率+市场宽度+动量)
  4. 组合: 波动率倒数加权 + 分层选股(CSI300/非CSI300)

对比: v5.0(静态池+ML) vs v6.0(动态池+ML+择时)
"""
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.0 全自主策略回测 (动态动量池 + ML + v5.3择时)")
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.0 动态动量池+ML选股回测')
