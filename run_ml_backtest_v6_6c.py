#!/usr/bin/env python
"""
v6.6C 波动率目标+保护 — 目标回撤-15%

基于v6.5A, 叠加:
  1. 激进择时 (v6.5A): min_exposure=0.20, exposure^1.5放大
  2. 个股止损: 从买入价跌10%自动卖出 (比v6.6A更紧)
  3. 渐进式组合止损: 同v6.6A
  4. 波动率目标控制: 目标年化波动率20%, 实际波动高时自动降低仓位
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.6C 波动率目标+保护 (激进择时 + 个股止损-10% + 渐进止损 + 波动率目标20%)")
    ...
    print(f"波动率目标: {TARGET_VOL:.0%}年化, 窗口{VOL_LOOKBACK}日")
    print("=" * 70)
    ...
    for i in range(1, len(dates)):
        ...
        # v6.6C: 波动率目标控制
        vol_multiplier = 1.0
        if len(enhanced_returns) >= VOL_LOOKBACK:
            recent_rets = np.array(enhanced_returns[-VOL_LOOKBACK:])
            realized_vol = np.std(recent_rets) * np.sqrt(250)
            if realized_vol > 0:
                vol_multiplier = min(1.0, TARGET_VOL / realized_vol)

        # 综合暴露度 = 择时 × 渐进止损 × 波动率目标
        final_exposure = market_position * dd_circuit_breaker * vol_multiplier
    ...
    # 保存结果
    result = {
        'version': 'v6.6c_vol_target_protection',
        'config': {
            'n_stocks': N_STOCKS,
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.6C 波动率目标+保护回测')
