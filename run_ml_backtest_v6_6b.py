#!/usr/bin/env python
"""
v6.6B 周度调仓+三重保护 — 目标回撤-15%

基于v6.6A三重保护, 叠加v6.5C的周度调仓:
  1. 激进择时 (v6.5A): min_exposure=0.20, exposure^1.5放大
  2. 个股止损: 从买入价跌12%自动卖出
  3. 渐进式组合止损: NAV回撤线性减仓
  4. 周度调仓(每5个交易日): 更快响应市场变化, ML训练仍为月度

保留v6.4B动态集中度 + v6.5A激进择时
"""
...
# 组合参数
# v6.4B: 动态参数 (默认值, 会被regime覆盖)
N_STOCKS = 30           # 默认30只 (动态覆盖: 25/30/40)
MAX_WEIGHT = 0.10       # 默认10% (动态覆盖: 15%/10%/5%)
...
VOL_LOOKBACK = 20       # 波动率回看天数(仅观察)
DD_THRESHOLD_1 = -0.99  # 禁用
DD_THRESHOLD_2 = -0.99  # 禁用
DD_RECOVERY = -0.99     # 禁用

# v6.6A 三重保护参数
STOCK_STOP_LOSS = -0.12         # 个股止损: 从买入价跌12%
# 渐进式组合止损 (线性插值)
PORTFOLIO_DD_START = -0.08      # 开始减仓的回撤阈值
PORTFOLIO_DD_END = -0.18        # 最大减仓的回撤阈值
PORTFOLIO_MIN_EXPOSURE = 0.10   # 最大减仓时的最低暴露

REGIME_BULL_THRESHOLD = 0.90    # exposure >= 0.90 = 牛市
REGIME_BEAR_THRESHOLD = 0.75    # exposure < 0.75 = 弱势
REGIME_BULL_STOCKS = 25         # 牛市: 25只集中
REGIME_BULL_MAX_W = 0.15        # 牛市: 15%上限
REGIME_NORMAL_STOCKS = 30       # 正常: 30只
REGIME_NORMAL_MAX_W = 0.10      # 正常: 10%上限
REGIME_BEAR_STOCKS = 40         # 弱势: 40只分散
REGIME_BEAR_MAX_W = 0.05        # 弱势: 5%上限
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.6B 周度+三重保护 (周度调仓 + 激进择时 + 个股止损 + 渐进组合止损)")
    ...
    print(f"渐进止损: DD>{PORTFOLIO_DD_START:.0%}开始减仓 → DD>{PORTFOLIO_DD_END:.0%}最低{PORTFOLIO_MIN_EXPOSURE:.0%}仓")
    print(f"个股止损: 买入价跌{STOCK_STOP_LOSS:.0%}自动卖出")
    print("=" * 70)
    ...
    # v6.6B: 周度调仓 (每5个交易日), ML训练仍为月度
    monthly_train_dates = set(d[:7] + '-01' for d in trade_rebalance_dates)  # 保留月度训练日期
    trade_rebalance_dates = [d for idx, d in enumerate(dates) if idx % 5 == 0]

    current_weights = {}  # 初始空仓, 等ML信号
    prev_weights = {}
    rb_idx = 0
    # v5.3 多信号择时 — v6.1: 更激进减仓 (min 0.30)
    # v6.5A: 激进择时, 最低仓位降到20% (原50%)
    timing_signal = MacroTimingSignal(min_exposure=0.20, max_exposure=1.00, ma_period=250)
    market_position = 1.0
    stop_loss_count = 0
    enhanced_returns = []
    original_returns = []
    ...
    for i in range(1, len(dates)):
        ...
        # v6.6A: 每日检查个股止损
        if current_weights and curr_date in price_pivot.index:
            curr_p_check = price_pivot.loc[curr_date]
            stopped_codes = []
            for code, w in list(current_weights.items()):
                if w < 0.001:
                    continue
                if code in stock_entry_prices and code in curr_p_check.index and pd.notna(curr_p_check[code]):
                    entry_px = stock_entry_prices[code]
                    if entry_px > 0:
                        ret_from_entry = curr_p_check[code] / entry_px - 1
                        if ret_from_entry < STOCK_STOP_LOSS:
                            stopped_codes.append(code)
            if stopped_codes:
                for code in stopped_codes:
                    del current_weights[code]
                    if code in stock_entry_prices:
                        del stock_entry_prices[code]
                stock_stop_count += len(stopped_codes)
                total_w = sum(current_weights.values())
                if total_w > 0:
                    current_weights = {k: v / total_w for k, v in current_weights.items()}
                stop_cost = len(stopped_codes) * 0.03 * TRANSACTION_COSTS['sell_commission']
                rebalance_cost += stop_cost
                total_tx_cost += stop_cost
                if len(stopped_codes) >= 2:
                    logger.info(f"  [{curr_date}] 个股止损: {len(stopped_codes)}只")
        ...
        while rb_idx < len(trade_rebalance_dates) and curr_date >= trade_rebalance_dates[rb_idx]:
            ...
            try:
                ...
                # Step 4: 训练ML — v6.6B: 仅月度训练, 周度调仓复用模型
                nearest_month = f'{rb_dt.year}-{rb_dt.month:02d}-01'
                is_monthly_train = (nearest_month in monthly_train_dates and
                                    abs((rb_dt - datetime.strptime(nearest_month, '%Y-%m-%d')).days) <= 7)
                if is_monthly_train and len(factor_history) >= MIN_TRAIN_MONTHS:
                    ...
                    if X_train is not None and len(X_train) >= 200:
                        logger.info(f"\n  [{rb_date}] 月度训练ML模型... ({len(active_features)}特征)")
                ...
                if ml_scorer.lgb_model is not None or ml_scorer.xgb_model is not None:
                    ...
                    # v6.4B: 根据宏观择时信号动态调整集中度
                    regime_exposure = timing_signal.calculate_exposure(index_prices, price_pivot, rb_date)
                    if regime_exposure >= REGIME_BULL_THRESHOLD:
                        dyn_n = REGIME_BULL_STOCKS
                        dyn_max_w = REGIME_BULL_MAX_W
                        regime_label = "BULL"
                    elif regime_exposure < REGIME_BEAR_THRESHOLD:
                        dyn_n = REGIME_BEAR_STOCKS
                        dyn_max_w = REGIME_BEAR_MAX_W
                        regime_label = "BEAR"
                    else:
                        dyn_n = REGIME_NORMAL_STOCKS
                        dyn_max_w = REGIME_NORMAL_MAX_W
                        regime_label = "NORMAL"
                    logger.info(f"  [{rb_date}] Regime={regime_label} (exp={regime_exposure:.2f}) → N={dyn_n}, MaxW={dyn_max_w:.0%}")

                    # 构建Top-N组合 (分层选股: CSI300 + 非CSI300)
                    csi300_codes = fetcher.get_csi300_constituents()
                    new_weights = portfolio_builder.build(
                        ml_scores, vol_dict,
                        n_stocks=dyn_n, max_weight=dyn_max_w,
                    ...
                    if rb_date in price_pivot.index:
                        rb_prices = price_pivot.loc[rb_date]
                        for code, w in current_weights.items():
                            if w > 0.001 and code not in stock_entry_prices:
                                if code in rb_prices.index and pd.notna(rb_prices[code]) and rb_prices[code] > 0:
                                    stock_entry_prices[code] = rb_prices[code]
                        # 清除已卖出股票的入场价
                        for code in list(stock_entry_prices.keys()):
                            if code not in current_weights or current_weights.get(code, 0) < 0.001:
                                del stock_entry_prices[code]
        ...
        # v6.5A: 放大择时信号 — exposure<0.85时加倍惩罚
        if market_position < 0.85:
            market_position = max(0.20, market_position ** 1.5)

        # v6.6A: 渐进式组合止损 (线性插值替代二元熔断)
        dd_from_peak = current_nav / peak_nav - 1
        if dd_from_peak >= PORTFOLIO_DD_START:
            # 回撤在阈值之上, 不减仓
            dd_circuit_breaker = 1.0
        elif dd_from_peak <= PORTFOLIO_DD_END:
            # 回撤超过最大阈值, 最低暴露
            dd_circuit_breaker = PORTFOLIO_MIN_EXPOSURE
        else:
            # 线性插值: DD_START(-8%) → 1.0, DD_END(-18%) → MIN_EXPOSURE(0.10)
            ratio = (dd_from_peak - PORTFOLIO_DD_START) / (PORTFOLIO_DD_END - PORTFOLIO_DD_START)
            dd_circuit_breaker = 1.0 - ratio * (1.0 - PORTFOLIO_MIN_EXPOSURE)
    ...
    # 保存结果
    result = {
        'version': 'v6.6b_weekly_triple_protection',
        'config': {
            'n_stocks': N_STOCKS,
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_5a.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.6B 周度+三重保护回测')
