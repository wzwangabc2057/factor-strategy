#!/usr/bin/env python
"""
v6.4B 动态集中度 — 基于v6.3 (年化35.83%, 回撤-25.78%)
...
def run_v6_backtest(start_date='2022-01-01', end_date='2025-12-31'):
    ...
    """
    print("=" * 70)
    print("v6.4B 动态集中度 (动态动量池 + ML + 择时 + 宽松回撤熔断)")
    ...
    # 保存结果
    result = {
        'version': 'v6.4b_dynamic_concentration',
        'config': {
            'n_stocks': N_STOCKS,
    ...
    output_path = os.path.join(BASE_DIR, 'factor_backtest_result_v6_4b.json')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='v6.4B 动态集中度回测')
